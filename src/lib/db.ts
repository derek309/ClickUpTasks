...
const projectToRow = (p: Project) => ({ id: p.id, client_id: p.clientId, name: p.name, description: p.description, assigned_to: p.assignedTo ?? [], follow_up_at: p.followUpAt ?? null, reviewed_at: p.reviewedAt ?? null, folder_id: p.folderId ?? null, position: p.position ?? 0, share_token: p.shareToken ?? null });
const rowToProject = (r: any): Project => ({ id: r.id, clientId: r.client_id, name: r.name, description: r.description ?? "", assignedTo: r.assigned_to ?? [], followUpAt: r.follow_up_at ?? null, reviewedAt: r.reviewed_at ?? null, folderId: r.folder_id ?? null, position: r.position ?? 0, shareToken: r.share_token ?? null });
