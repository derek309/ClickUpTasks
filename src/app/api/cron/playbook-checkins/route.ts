...
import { runPlaybookStallCheck } from "@/lib/playbookCheckinsServer";

// Daily stall-check for the Playbook (Owner Growth Plan) — see
// playbookCheckinsServer.ts. Same 3-way cron auth as the other crons.
...
async function run(req: NextRequest) {
  ...
  const result = await runPlaybookStallCheck();
