...
// Best-effort client suggestion for the extension's popup — either from a
// Gmail sender's email, or (review mode) directly from the domain of the
// page being reviewed. Tries an exact (case-insensitive) email match
// against an existing Contact first when an email is given; either way
// falls back to matching the domain against a visible client's contacts'
// domains — but only when exactly one client matches, so an ambiguous
// domain (shared by two different clients) doesn't guess wrong. Both paths
// are restricted to the caller's visible clients so a VA's token can't
