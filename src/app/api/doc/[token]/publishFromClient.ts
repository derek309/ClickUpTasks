...
// resolving, then the publish itself. An HTML review also sends the version the
// client looked at (body) and their rewording, page by page.
export async function publishFromClient(
  ...
  const { html, baseVersion, body, edits } = read.body;
  const outcome = await clientPublish(scope, kind, html, baseVersion, { body, edits });
  if (!outcome.ok) {
